...
export async function createSubmission(
  ...
  // DocuSeal /api/submissions/emails returns an array of submitter objects,
  // each containing submission_id. Transform to our expected shape.
  const rawSubmitters = (await res.json()) as Array<{
    id: number
  ...
  if (!rawSubmitters.length) {
    throw new Error('DocuSeal returned empty submitters array')
  }

  return {
    id: rawSubmitters[0].submission_id,
    submitters: rawSubmitters.map((s) => ({
      id: s.id,
      email: s.email,
      name: s.name,
      status: s.status,
    ...
    status: 'pending',
    createdAt: rawSubmitters[0].created_at,
  }
}
